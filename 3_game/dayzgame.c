modded class DayZGame extends CGame {
	...
	PVEZ_Config pvez_Config;
	PVEZ_Zones pvez_Zones;
	PVEZ_LawbreakersMarkers pvez_LawbreakersMarkers;
	PVEZ_LawbreakersRoster pvez_LawbreakersRoster;
	PVEZ_Bounties pvez_Bounties;
	...
	void PVEZ_SendActiveZonesToClient(DayZPlayer dzp = NULL) {
		if (IsServer()) {
			Param1<array<PVEZ_Zone>> zonesData = new Param1<array<PVEZ_Zone>>(pvez_Zones.activeZones);
			if (dzp)
		...
		if (IsClient()) {
			switch (rpc_type) {
				case PVEZ_RPC.UPDATE_LAWBREAKERS_MARKERS:
					...
					Param1<array<PVEZ_LawbreakerMarker>> data2 = new Param1<array<PVEZ_LawbreakerMarker>>(NULL);
			...
			switch (rpc_type) {
				...
				case PVEZ_RPC.UPDATE_ZONES:
					Param1<array<PVEZ_Zone>> data5 = new Param1<array<PVEZ_Zone>>(NULL);
					if (!ctx.Read(data5)) {
				...
				case PVEZ_RPC.UPDATE_BOUNTIES:
					Param2<bool, array<PVEZ_BountyItemData>> data6 = new Param2<bool, array<PVEZ_BountyItemData>>(false, NULL);
					if (!ctx.Read(data6)) {
				...
				case PVEZ_RPC.ADMIN_ZONES_DATA_REQUEST:
					Param1<array<PVEZ_Zone>> data7 = new Param1<array<PVEZ_Zone>>(pvez_Zones.staticZones);
				...
				case PVEZ_RPC.ADMIN_LAWBREAKERS_DATA_REQUEST:
					array<Man> players = new array<Man>;
					GetPlayers(players);
					Param2<array<PVEZ_Lawbreaker>, array<Man>> data8 = new Param2<array<PVEZ_Lawbreaker>, array<Man>>(pvez_LawbreakersRoster.lbDataBase, players);
				...
				case PVEZ_RPC.ADMIN_UPDATE_LAWBREAKERS:
					Param1<array<PVEZ_Lawbreaker>> data10 = new Param1<array<PVEZ_Lawbreaker>>(NULL);
					...
					GetPlayers(players2);
					Param2<array<PVEZ_Lawbreaker>, array<Man>> data11 = new Param2<array<PVEZ_Lawbreaker>, array<Man>>(pvez_LawbreakersRoster.lbDataBase, players2);
